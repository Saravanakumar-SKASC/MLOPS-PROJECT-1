...
class DataPreprocessing:
    ...
    def preprocess_data(self,df):
        try:
            ...
            logger.info("Applying label encoding on categorical columns")
            ...
            logger.info("Saving the label encoder mappings")
            ...
            raise CustomException(f"Error while preprocess data:{str(e)}")
    def balance_data(self,df):
        try:
            ...
            raise CustomException(f"Error while balancing data: {str(e)}")
    ...
    def select_features(self,df):
        try:
            ...
            top_10_features = top_features_df['Feature'].head(num_features_to_select).values
            ...
            raise CustomException(f"Error while feature selection: {str(e)}")
    ...
    def save_data(self,df, file_path):
        try:
            ...
            logger.error(f"Error during saving step {e}")
            raise CustomException(f"Error while saving data: {str(e)}")
        
    def process(self):
        try:
            logger.info("Loading the data from RAW")
            ...
            raise CustomException(f"Error while data preprocesing pipeline:{str(e)}")
